...
if(task_=="regression"){
     ...
     ggsave(paste0(save_plots_path,sim_,"_",task_,"_",n_,"_",mvn_dim_,"_n_tree_",n_tree_,".pdf"), plot = main_plot,
        ...
        # Just a decoy to get a legend
        rmse_plot_legend <- result_df %>% filter(metric == "logloss_test") %>%
                mutate(mvn_dim = as.factor(mvn_dim)) %>%
                ggplot()+
        ...
        pi_range <- result_df %>% dplyr::group_by(metric) %>% summarise(min = min(value), max = max(value)) %>% filter(metric=='p_cr_test') %>% select(min,max) %>% c %>% unlist()
