...
cv_matrix <- function(cv_element_,
          ...
          if(task_=="regression"){
            ...
            comparison_metrics <- rbind(comparison_metrics, data.frame(metric = "acc_train",
                                                                       value = acc(y_true = y_true_train[,i_],
                                                                                  y_hat = ifelse(pnorm(colMeans(bart_models[[i_]]$yhat.train))>0.5,1,0)),
                                                                       model = "BART", fold = i,
                                                                       mvn_dim = i_))

            comparison_metrics <- rbind(comparison_metrics, data.frame(metric = "acc_test",
                                                                       value = acc(y_true = y_true_test[,i_],
                                                                                          y_hat = ifelse(pnorm(colMeans(bart_models[[i_]]$yhat.test))>0.5,1,0)),
                                                                       model = "BART", fold = i,
                                                                       mvn_dim = i_))
     ...
     if(task_ == "regression"){
       ...
       comparison_metrics <- rbind(comparison_metrics, data.frame(metric = "logloss_train",
                                                                  value = logloss(y_true = y_true_train[,i_],
                                                                                  y_hat = pnorm(rowMeans(mvbart_mod$y_hat[,i_,]))),
                                                                  model = "mvBART", fold = i,
                                                                  mvn_dim = i_))

       comparison_metrics <- rbind(comparison_metrics, data.frame(metric = "logloss_test",
                                                                  value = logloss(y_true = y_true_test[,i_],
                                                                                  y_hat = pnorm(rowMeans(mvbart_mod$y_hat_test[,i_,]))),
                                                                  model = "mvBART", fold = i,
                                                                  mvn_dim = i_))
       ...
       comparison_metrics <- rbind(comparison_metrics, data.frame(metric = "acc_train",
                                                                  value = acc(y_true = y_true_train[,i_],
                                                                              y_hat = ifelse(pnorm(rowMeans(mvbart_mod$y_hat[,i_,]))>0.5,1,0)),
                                                                  model = "mvBART", fold = i,
                                                                  mvn_dim = i_))

       comparison_metrics <- rbind(comparison_metrics, data.frame(metric = "acc_test",
                                                                  value = acc(y_true = y_true_test[,i_],
                                                                              y_hat = ifelse(pnorm(rowMeans(mvbart_mod$y_hat_test[,i_,]))>0.5,1,0)),
                                                                  model = "mvBART", fold = i,
                                                                  mvn_dim = i_))
