...
results_path <- "AOAS_code/subart_simulations/"
save_plots_path <- "AOAS_code/subart_simulations/"
...
for(model in models){
     ...
     file_path <- paste0(results_path, "seed_", seed_, "_", model, "_", sim_, "_", task_,
...
for(m in names(metrics)){
     ...
     p <- ggplot(dat, aes(x = model, y = value, col = model)) +
          ...
          scale_color_manual(values = c("suBART"="#F8766D","BART"="#00BA38",
...
# Legend plot
# -------------------------------
legend_plot <- ggplot(result_df %>% filter(metric == names(metrics)[1]),
...
summary_corr_comparison <- bind_rows(summary_df_corr_cov,
